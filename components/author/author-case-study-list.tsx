...
export function AuthorCaseStudyList() {
  ...
  useEffect(() => {
    queueMicrotask(() => {
      const ids = listIds("case-study-draft");
      const next = ids.map((id) => {
        const w = readWrapped<CaseStudyDocument>("case-study-draft", id);
        return {
          id,
  ...
  if (rows.length === 0) {
    return (
      <p className="text-muted-foreground text-sm">
        No drafts yet. Create a case study to get started.
      </p>
    );
  }
          ...
          <TableRow key={r.id}>
            ...
            <TableCell className="text-right">
              <Button asChild size="sm" variant="ghost">
                <Link href={`/author/case-studies/${r.id}`}>Edit</Link>
              </Button>
            </TableCell>
          </TableRow>
        ))}
